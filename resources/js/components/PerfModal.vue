  ...
  <div v-if="modelValue" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60" @click="close">
    <div class="bg-[#141418] rounded-xl p-6 w-full max-w-md shadow-xl" @click.stop>
      ...
      <div class="max-h-40 overflow-y-auto space-y-2 mb-4">
        <label v-for="p in performers" :key="p.id" class="flex items-center gap-2 text-sm">
          <input type="checkbox" :value="p.id" v-model="localPerformers" />
          {{ p.name }}
        </label>
      </div>
      ...
      <div class="flex justify-end gap-3">
        ...
        <button type="button" @click="confirm" class="bg-pink-500 px-4 py-2 rounded font-bold hover:bg-pink-600">
          Potvrdit
        </button>
      </div>
    </div>
