<template>
  <div class="p-4 bg-surface rounded-xl w-72 space-y-4">
    ...
    <div v-if="showFollowers">
      <h3 class="text-app font-semibold mb-2">Followers ({{ followers.length }})</h3>
      <div class="space-y-2 max-h-40 overflow-y-auto no-scrollbar">
        <div v-for="user in followers" :key="user.id"
          class="p-2 bg-surface-hover rounded hover:bg-surface-active transition cursor-pointer">
          {{ user.username }}
        </div>
      </div>
    </div>
    ...
    <div>
      <h3 class="text-app font-semibold mb-2">Following ({{ following.length }})</h3>
      <div class="space-y-2 max-h-40 overflow-y-auto no-scrollbar">
