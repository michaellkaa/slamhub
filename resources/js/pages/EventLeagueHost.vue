  ...
  <div class="min-h-screen bg-[#0f0f12] text-white px-4 py-6 flex justify-center">
    <div class="w-full max-w-3xl">
      <div class="flex items-center justify-between mb-6">
        <button @click="$router.back()" class="text-pink-500 hover:underline text-lg">← Zpet</button>
        <div class="text-sm text-white/60">League spider (host)</div>
      </div>
      <div class="rounded-2xl bg-[#121218] p-5 space-y-5">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
          <div v-for="slot in localSlots" :key="slot.id" class="bg-[#0f0f12] rounded-xl p-3">
            <div class="text-xs text-white/60 mb-2">Soutezici {{ slot.id }}</div>
        ...
        <div class="space-y-3">
          <div v-for="m in matches" :key="m.id" class="bg-[#0f0f12] rounded-xl p-3">
            ...
            <select v-model="roundRobin[m.id]" class="w-full rounded-lg bg-[#1d1d21] p-2 text-sm">
              <option :value="null">Vyber viteze</option>
        ...
        <div class="bg-[#0f0f12] rounded-xl p-3">
          <div class="text-sm text-white/70 mb-2">Finale: 1. vs vitez 2. kola</div>
          <select v-model="finalWinner" class="w-full rounded-lg bg-[#1d1d21] p-2 text-sm">
            <option :value="null">Vyber viteze ligy</option>
        ...
        <button @click="saveLeague" class="w-full rounded-xl bg-pink-500 hover:bg-pink-600 py-3 font-bold">Ulozit spider</button>
      </div>
    </div>
  </div>
</template>
