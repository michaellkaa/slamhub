<template>
    <div class="bg-[#0f0f12] w-full min-h-screen flex flex-col lg:flex-row overflow-hidden">
        ...
        <div class="flex-1 flex gap-6 lg:p-6 overflow-hidden no-scrollbar">
            <div class="flex-1  lg:p-4 overflow-y-auto lg:max-h-[90vh] max-h-full no-scrollbar touch-pan-y overscroll-y-contain snap-y snap-mandatory pb-36 lg:pb-0"
                style="scroll-snap-type: y mandatory;">
                <div class="lg:hidden fixed top-0 left-0 right-0 z-[100] px-4 pt-3 pb-3 pointer-events-none">
                    <div class="pointer-events-auto max-w-md mx-auto w-full">
                        <TopSearch />
                    </div>
                </div>
                ...
                <div v-for="video in videos" :key="video.id"
                    ...
                    <div class="relative w-full h-[100svh] lg:h-[calc(100vh-3rem)] overflow-hidden group">
                        <div class="absolute inset-0 bg-black">
                            <video :src="video.video_url"
                                class="absolute inset-0 w-full h-full object-cover transition group-hover:scale-[1.01]"
                                autoplay muted loop playsinline></video>
                            <div class="absolute inset-0 bg-gradient-to-t from-black/75 via-transparent to-transparent">
                            </div>

                            <div
                                class="absolute right-3 top-1/2 -translate-y-1/2 flex flex-col items-center gap-4 z-10">
                                ...
                                <button type="button" class="inline-flex flex-col items-center transition"
                                    ...
                                    <svg viewBox="0 0 24 24" class="w-7 h-7 lg:w-6 lg:h-6" fill="currentColor"
                                        aria-hidden="true">
                                        <path
                                ...
                                <button type="button" class="inline-flex flex-col items-center text-white"
                                    ...
                                    <svg viewBox="0 0 24 24" class="w-7 h-7 lg:w-6 lg:h-6" fill="none"
                                ...
                                <button type="button" class="inline-flex flex-col items-center text-white"
                                    @click="shareVideo(video)" aria-label="Share">
                                    <svg viewBox="0 0 24 24" class="w-7 h-7 lg:w-6 lg:h-6" fill="none"
                                        stroke="currentColor" stroke-width="2" aria-hidden="true">
                        ...
                        <div class="absolute bottom-40 left-4 right-16 text-white z-20">
            ...
            <div class="hidden w-80 lg:flex flex-col gap-6 overflow-y-auto max-h-[90vh]">
                ...
                <div class="rounded-xl bg-[#121216] border border-white/10 p-4">
                    ...
                    <div v-else class="space-y-3">
                        <div v-for="user in suggestedUsers" :key="'suggest-' + user.id" class="flex items-center gap-3">
                            <img :src="user.profile_pic_url || '/images/default-avatar.png'"
                            ...
                            <button type="button" class="text-xs rounded-lg px-2.5 py-1.5 transition"
                ...
                <div class="rounded-xl bg-[#121216] border border-white/10 p-4">
                    ...
                    <div v-else class="space-y-3">
                        <button v-for="event in upcomingEvents" :key="'event-' + event.id"
        ...
        <div v-if="activeCommentItem" class="fixed inset-0 z-[70] flex items-center justify-center bg-black/70 p-4"
            ...
            <div class="w-full max-w-lg rounded-2xl border border-white/10 bg-[#121216]">
                <div class="flex items-center justify-between border-b border-white/10 px-4 py-3">
                    ...
                    <button class="rounded-lg px-2 py-1 text-white/70 hover:bg-white/10"
                        @click="closeCommentsModal">Close</button>
                </div>

                <div class="max-h-[55vh] overflow-y-auto px-4 py-3 space-y-3">
                    <div v-if="activeCommentItem.commentsLoading" class="text-xs text-white/50">Loading comments...
                    </div>
                    <div v-else-if="!activeCommentItem.comments?.length" class="text-xs text-white/50">No comments yet.
                    </div>
                    <div v-else class="space-y-2">
                        <div v-for="comment in activeCommentItem.comments" :key="comment.id"
                            ...
                            <button type="button" class="mt-1 text-[11px] text-white/60 hover:text-white"
                                @click="startReply(comment)">
                                Reply
                            </button>
                            <div v-if="comment.replies?.length"
                                class="mt-2 ml-4 space-y-2 border-l border-white/10 pl-3">
                ...
                <form class="border-t border-white/10 p-3 flex gap-2"
                    @submit.prevent="submitComment(activeCommentItem)">
                ...
                <div v-if="replyTargetId" class="px-3 pb-3">
                    <button class="text-xs text-white/60 hover:text-white" @click="replyTargetId = null">Cancel
                        reply</button>
                </div>
            </div>
        </div>
