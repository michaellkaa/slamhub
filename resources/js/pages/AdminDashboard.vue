<template>
  <div class="min-h-screen bg-app text-app px-3 md:px-6 py-6 md:py-10 pb-24">
    <div class="w-full max-w-6xl mx-auto">
      ...
      <template v-else>
        <nav class="flex gap-1 overflow-x-auto no-scrollbar mb-6 border-b border-app pb-1">
          <button
            v-for="tab in tabs"
            :key="tab.key"
            type="button"
            @click="activeTab = tab.key"
            class="px-3 py-2 text-sm font-medium rounded-t-lg transition whitespace-nowrap"
        ...
        <section v-else-if="activeTab === 'overview'" class="space-y-6">
          <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div v-for="card in overviewCards" :key="card.label" class="rounded-xl border border-app bg-surface p-4">
              <div class="text-xs text-app-muted">{{ card.label }}</div>
        ...
        <section v-else-if="activeTab === 'users'" class="space-y-4">
          <div class="flex flex-col md:flex-row gap-3">
            <input
              ...
              class="flex-1 h-10 px-3 rounded-md bg-surface text-app border border-app focus:outline-none"
          ...
          <div v-else class="rounded-xl border border-app overflow-hidden">
            <div class="overflow-x-auto">
              <table class="w-full text-sm">
                <thead class="bg-surface text-app-muted text-left">
                  <tr>
                    <th class="px-3 py-2 font-medium">Uživatel</th>
                ...
                <tbody>
                  <tr v-for="user in users" :key="user.id" class="border-t border-app">
                    <td class="px-3 py-3">
                      <div class="flex items-center gap-3 min-w-[180px]">
                        <img :src="user.profile_pic_url" class="w-8 h-8 rounded-full object-cover" alt="" />
                        <div>
                          <div class="font-medium">{{ user.name }}</div>
                          <div class="text-xs text-app-muted">@{{ user.username }}</div>
                        </div>
                      </div>
                    </td>
                    <td class="px-3 py-3">
                      <select
                        :value="user.role"
                        class="h-9 px-2 rounded-md bg-surface border border-app text-app"
                    ...
                    <td class="px-3 py-3 text-app-muted">{{ user.points }}</td>
                    <td class="px-3 py-3 text-right">
                      <a :href="`/profile/${user.username}`" class="text-xs text-pink-400 hover:text-pink-300">Profil</a>
                    </td>
                  </tr>
                </tbody>
              </table>
            </div>
            <div v-if="usersMeta.last_page > 1" class="flex items-center justify-between px-3 py-2 border-t border-app text-xs text-app-muted">
            ...
            <table class="w-full text-sm">
              <thead class="bg-surface text-app-muted text-left">
                <tr>
                  ...
                  <th class="px-3 py-2 font-medium">Autor</th>
                  <th class="px-3 py-2 font-medium">Status</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="video in videos" :key="video.id" class="border-t border-app">
                  <td class="px-3 py-3">{{ video.title || 'Bez názvu' }}</td>
                  <td class="px-3 py-3 text-app-muted">@{{ video.user?.username }}</td>
                  <td class="px-3 py-3">
                    <select
                      :value="video.status"
                      class="h-9 px-2 rounded-md bg-surface border border-app text-app"
        ...
        <section v-else-if="activeTab === 'events'" class="space-y-4">
          ...
          <div v-else class="space-y-3">
            ...
            >
              <div class="font-medium">{{ event.title || 'Bez názvu' }}</div>
